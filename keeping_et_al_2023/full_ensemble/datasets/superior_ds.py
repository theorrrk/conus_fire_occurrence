...
def main():
    ...
    paths = [direc + f'output_summary_{code}.csv' for code in superior_codes]
    thresh_row_list = []
    variables_list = []
    for path in paths:
        df = pd.read_csv(path)
    ...
    for i in range(len(variables_list)):
        ...
        if i == 0:
            ...
            prob += (a * da ** b).to_numpy()
    prob = prob / len(variables_list)

    output = xr.Dataset(data_vars = {'p': (['lat', 'lon', 'date'], prob)},
                        coords = {'lat': ds.lat.data, 
                                  'lon': ds.lon.data,
                                  'date': ds.date.data})

    output.to_netcdf('/rds/general/user/tk22/home/fire_genesis/ensemble_superior_members.nc')
    return
